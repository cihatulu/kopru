-- KÖPRÜ — Kısmi sevkiyatta özel talep açıklaması (custom_description) ve
-- fiyat farkı (price_difference) alanlarının çocuk siparişe eksiksiz aktarılması.
...
begin
  ...
  -- Çocuk sipariş için başlangıç logu ekle
  insert into public.order_status_logs (order_id, from_status, to_status, actor_user_id, actor_org_id, note)
  ...
  for v_req in select * from jsonb_array_elements(p_items) loop
    ...
    insert into public.order_items (
      ...
      v_item.product_snapshot, v_item.custom_description, v_item.price_difference
    )
