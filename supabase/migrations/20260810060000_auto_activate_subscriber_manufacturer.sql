-- KÖPRÜ — Üye Üretici Otomatik Aktif Bağlantı
--
-- Mevcut kural: hedef abone → pending (karşı taraf onaylamalı)
-- Yeni kural:
--   Perakendeci + üye üretici → active (onay gereksiz; üretici zaten platformda, satmak istiyor)
--   Üretici + üye perakendeci → pending (üretici müşteri listesini kontrol etmeli)
--
-- Gerekçe: perakendeci "Anadolu Mobilya" VKN'sini ekleyince üye olduğu için
-- pending kalıyordu. Üretici onaylayana kadar ne Tedarikçilerim'de görünüyor
-- ne de katalog erişilebiliyordu. Platform amacı B2B satış; gereksiz sürtünme.

-- ─── 1. Mevcut pending → active düzeltmesi ──────────────────────────────────
-- Perakendicinin başlattığı, hedefi subscriber manufacturer olan pending ilişkiler
-- artık active hale getirilir.
...
where r.status = 'pending'
  and r.initiated_by_org_id = r.retailer_org_id   -- perakendeci başlattı
  and exists (
    select 1 from public.organizations o
    where o.id = r.manufacturer_org_id
      and o.is_subscriber = true
  );

-- ─── 2. add_counterparty fonksiyonu güncelleme ───────────────────────────────
-- Tek değişiklik: v_status hesaplama mantığı.
...
begin
  ...
  -- ── YENİ KURAL ─────────────────────────────────────────────────────────────
  -- Perakendeci + üye üretici → active (üretici onayı gerekmez; platform satış odaklı)
  -- Üretici + üye perakendeci → pending (üretici kendi müşteri listesini kontrol eder)
  -- Diğer durumlar (hedef misafir) → active (değişmedi)
  -- ─────────────────────────────────────────────────────────────────────────────
  v_status := case
    when v_target.is_subscriber and v_me.kind = 'manufacturer' then 'pending'
    else 'active'
  end;
