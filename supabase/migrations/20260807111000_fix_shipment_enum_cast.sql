-- KÖPRÜ — düzeltme: ship_order_atomic içinde enum dönüşümü
--
-- Hata: `case when ... then 'shipped' else 'partially_shipped' end` ifadesi
-- `text` üretir; Postgres onu `order_status` enum'una otomatik çeviremez ve
-- çalışma anında `42804 column "status" is of type order_status but expression
-- is of type text` verir. Canlı sevkiyat testinde yakalandı.
--
-- Çözüm: CASE dallarını açıkça enum'a cast et ve sonucu bir değişkende topla.
...
begin
  ...
  if p_items is null or jsonb_typeof(p_items) <> 'array' or jsonb_array_length(p_items) = 0 then
    ...
    values (p_order_id, v_from, 'shipped'::public.order_status, public.get_my_user_id(), v_me);
    return p_order_id;
  end if;
  ...
  for v_req in select * from jsonb_array_elements(p_items) loop
    ...
    -- Kökün `quantity` alanı HER ZAMAN kalan miktardır.
    ...
    -- KATMAN 3 çocuğa taşınır; kopyalama tümüyle sunucu içinde, üretici görmez.
  ...
  -- Kök + çocuklar toplamı sabit kalır; CARİ DEFTERE DOKUNULMAZ.
  ...
  values (p_order_id, v_from, v_to, public.get_my_user_id(), v_me, 'Sevkiyat: ' || v_child_no);
