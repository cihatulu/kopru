-- KÖPRÜ — İade onaylandığında (confirm_return_atomic) iade tutarına
-- ve cari hareketinin items_snapshot dökümüne özel talep açıklaması (custom_description)
-- ve fiyat farkının (price_difference) dahil edilmesi.
...
begin
  ...
  insert into public.order_status_logs (
    ...
    v_req.order_id, v_order.status, v_target_status,
    public.get_my_user_id(), v_me,
    'İade onaylandı: ' || v_amount::text || ' TL'
  );
