...
export function GroupEditor({
  ...
  return (
    <Modal
      label={`Grubu düzenle: ${group.name}`}
      panelClassName="flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl bg-white p-6 shadow-xl"
      onClose={onBack}
      ...
      {confirmDelete ? (
        <div className="mt-5 rounded-xl bg-red-50 p-4 ring-1 ring-inset ring-red-200">
          <p className="text-sm text-red-800">
            <strong>{group.name}</strong> grubu silinsin mi? Ürünler silinmez, yalnız gruptan çıkar.
          </p>
