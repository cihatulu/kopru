...
export function ScopeDialog({ member, customers, pending, onClose, onSave }: Props) {
  ...
  return (
    <Modal
      label={'Müşteri kapsamı'}
      panelClassName={
        'flex max-h-[90vh] w-full max-w-md flex-col rounded-xl bg-white p-6 shadow-xl'
      }
      onClose={onClose}
    ...
    >
      ...
      <p className="mt-1 text-sm text-slate-500">
        {member.fullName ?? member.userCode} yalnız işaretlenen müşterilerin siparişlerini, carisini
        ve servis kayıtlarını görür.
      </p>
      ...
      {scope.isPending ? (
        ...
        <div className="mt-4 min-h-0 flex-1 space-y-1 overflow-y-auto">
          {customers.length === 0 && (
            <p className="py-6 text-center text-sm text-slate-500">Henüz aktif müşteriniz yok.</p>
          )}
      ...
      {selected.size === 0 && customers.length > 0 && (
        <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2.5 text-xs leading-relaxed text-amber-900">
          Hiçbir müşteri seçili değil. Bu şekilde kaydederseniz personel hiçbir müşteriyi göremez.
        </p>
      )}
