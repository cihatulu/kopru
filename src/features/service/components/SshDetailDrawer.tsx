...
export function SshDetailDrawer({ sshId, myOrgId, onClose }: Props) {
  ...
  return (
    <Modal
      label="Servis talebi detayı"
      backdropClassName="fixed inset-0 z-50 flex justify-end bg-slate-900/40"
      panelClassName="h-full w-full max-w-lg overflow-y-auto bg-white p-6 shadow-xl"
      onClose={onClose}
    >
      {detail.isPending && (
      ...
      {d && (
        <div className="space-y-6">
          ...
          <section>
            <h3 className="mb-3 text-xs font-semibold uppercase text-slate-500">Durum geçmişi</h3>
          ...
          {!closed && (
            <section className="space-y-3 border-t border-slate-100 pt-4">
              ...
              <div className="flex flex-wrap justify-end gap-2">
                <Button variant="ghost" loading={advance.isPending} onClick={() => move('iptal')}>
                  İptal et
                </Button>
