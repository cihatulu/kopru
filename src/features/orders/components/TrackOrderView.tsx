...
export function TrackOrderView({ order }: { order: TrackedOrder }) {
  ...
  return (
    <div className="space-y-6">
              ...
              return (
                ...
                >
                  ...
                  {isCancelled ? (
                    ...
                    const shipmentCalculations = s.items.map((item) => {
                      ...
                      const initialTotalP = unitP * item.quantity;
                      const netDeliveredTotalP = unitP * deliveredQty;
                    ...
                    const initialShipmentTotal = shipmentCalculations.reduce((sum, c) => sum + c.initialTotalP, 0);
                    const netShipmentDeliveredTotal = shipmentCalculations.reduce((sum, c) => sum + c.netDeliveredTotalP, 0);
                    const shipmentReturnTotal = initialShipmentTotal - netShipmentDeliveredTotal;
                    const hasReturns = shipmentReturnTotal > 0;
                    ...
                    return (
                      <div>
                        <div className="mt-1">
                          <table className="w-full text-xs">
                            <thead>
                              <tr className="border-b border-slate-100 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                                ...
                                <th className="pb-2 text-center">Teslim Adedi</th>
                                <th className="pb-2 text-right">Birim Fiyat</th>
                                <th className="pb-2 text-right">Teslim Tutarı</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50">
                              {shipmentCalculations.map((calc, idx) => {
                                const { item, retQty, deliveredQty, unitP, initialTotalP, netDeliveredTotalP } = calc;
                                ...
                                return (
                                  <tr key={`${item.productId ?? item.name}-${idx}`}>
                                    <td className="py-2.5 pr-2">
                                      ...
                                      {retQty > 0 && (
                                        <span className="inline-flex items-center gap-1 mt-1 rounded bg-amber-50 px-1.5 py-0.5 text-[10px] font-bold text-amber-900 border border-amber-200/60">
                                          ↩ {retQty} Adet İade Edildi ({item.quantity} Sevkten Kalan)
                                        </span>
                                      )}
                                    </td>
                                    <td className="py-2.5 text-center whitespace-nowrap">
                                      {retQty > 0 ? (
                                        <div>
                                          <span className="font-bold text-slate-900">{deliveredQty} Adet</span>
                                          <span className="text-[10px] text-slate-400 block line-through">({item.quantity} Sevk)</span>
                                        </div>
                                      ) : (
                                    ...
                                    <td className="py-2.5 text-right whitespace-nowrap">
                                      {retQty > 0 ? (
                                        <div>
                                          <span className="font-bold text-slate-900">{formatMoney(netDeliveredTotalP)}</span>
                                          <span className="text-[10px] text-slate-400 block line-through">{formatMoney(initialTotalP)}</span>
                                        </div>
                                      ) : (
                                        <span className="font-bold text-slate-900">{formatMoney(netDeliveredTotalP)}</span>
                                      )}
                          ...
                          <div className="mt-2.5 flex flex-wrap items-center justify-between border-t border-slate-100 pt-2 text-xs gap-2">
                            {hasReturns ? (
                              <div className="space-y-0.5">
                                <div className="flex items-center gap-2">
                                  <span className="text-slate-400 text-[11px]">İlk Sevk: <span className="line-through">{formatMoney(initialShipmentTotal)}</span></span>
                                  <span className="text-amber-800 text-[11px] font-semibold">İade: −{formatMoney(shipmentReturnTotal)}</span>
                                </div>
                                <span className="font-bold text-slate-800 text-xs">
                                  Teslim Edilen Net Tutar:{' '}
                                  <span className="font-mono text-emerald-700 font-bold text-sm">
                                    {formatMoney(netShipmentDeliveredTotal)}
                                  </span>
                                </span>
                              </div>
                            ) : (
      ...
      {/* ────────────────────────────────────────────────────────── */}
      {/* 2. FAZ: NİHAİ MÜŞTERİ EV TESLİMATI & MONTAJ TAKİBİ */}
      {/* ────────────────────────────────────────────────────────── */}
      {order.customer_deliveries && order.customer_deliveries.length > 0 && (
        <div className="mt-8 pt-6 border-t-2 border-dashed border-emerald-300/80">
          <div className="mb-4">
            <div className="flex items-center gap-2">
              <span className="flex size-7 items-center justify-center rounded-lg bg-emerald-600 text-white text-xs font-black">
                2
              </span>
              <h2 className="text-base font-black text-slate-900 tracking-tight">
                Müşteri Adresine Teslimat & Montaj Takibi
              </h2>
            </div>
            <p className="text-xs text-slate-500 mt-1 pl-9">
              Fabrika tedarik ve sevkiyat süreci tamamlanmış olup; mağazanız tarafından adresinize yapılacak teslimat ve montaj süreci aşağıda yer almaktadır.
            </p>
          </div>
          ...
          <div className="space-y-4">
            {order.customer_deliveries.map((deliv, idx) => (
              <div
                key={deliv.id || idx}
                className="rounded-2xl border-2 border-emerald-500/30 bg-gradient-to-br from-emerald-50/80 via-white to-teal-50/50 p-5 shadow-sm space-y-4"
              >
                <div className="flex items-center justify-between gap-2 border-b border-emerald-100 pb-3">
                    ...
                    <div>
                      <h3 className="text-sm font-black text-emerald-950">
                        {deliv.status === 'delivered'
                          ? 'Evinize Teslim Edildi & Montaj Tamamlandı'
                          : deliv.status === 'shipped'
                  ...
                  <span className="inline-flex items-center rounded-full px-3 py-1 text-[11px] font-black bg-emerald-100 text-emerald-800 border border-emerald-200 uppercase tracking-wider">
                    {deliv.status === 'delivered' ? '✓ Teslim Edildi' : 'Randevu Aktif'}
                  </span>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-white/90 rounded-xl p-3.5 border border-emerald-100/80">
                  <div>
                ...
                {deliv.items && deliv.items.length > 0 && (
                  <div className="bg-white/90 rounded-xl p-3.5 border border-emerald-100/80">
                    <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">
                      Bu Randevuda Teslim Edilecek Ürünler
                    </span>
                    <ul className="divide-y divide-emerald-50 text-xs">
                      {deliv.items.map((di, dIdx) => (
                        <li key={dIdx} className="py-1.5 flex items-center justify-between">
                ...
                {deliv.customer_address && (
                  <div className="bg-white/90 rounded-xl p-3.5 border border-emerald-100/80">
                    <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">
                      Teslimat & Montaj Adresi
                    </span>
                    <span className="font-medium text-slate-800 leading-relaxed block mt-0.5 text-xs">
                      📍 {deliv.customer_address}
                    </span>
                  </div>
                )}
                ...
                {deliv.notes && (
                  <div className="bg-emerald-50/60 rounded-xl p-3 border border-emerald-100 text-xs text-emerald-900 font-medium">
